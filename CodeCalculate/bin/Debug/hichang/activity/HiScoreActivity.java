...
public class HiScoreActivity extends Activity {
	 ...
	 * 排行榜的成绩数字图片
	 */
	private ImageView firstLeftNum, firstRightNum, secLeftNum, secRightNum,
			thirdLeftNum, thirdRightNum;
	/**
	 * 排行榜，鼓励句子，鼓励图片
	 */
	 ...
	 * 排行榜的数字标号
	 */
	private ImageView rankBigFirst, rankSmallFirst, rankBigSec, rankSmallSec,
			rankBigThird, rankSmallThird;
	/**
	 * 打分的十位和个位
	 */
	...
	public void initPicture() {
		rankinglist[0] = BitmapFactory.decodeResource(resources,
				R.drawable.score_rank_list);
		rankinglist[1] = BitmapFactory.decodeResource(resources,
				R.drawable.score_record_break);

		approveSentences[0] = BitmapFactory.decodeResource(resources,
				R.drawable.score_go_on);
		approveSentences[1] = BitmapFactory.decodeResource(resources,
				R.drawable.score_excellent);

		for (int i = 0; i < 6; i++) {
			approveImages[i] = BitmapFactory.decodeResource(resources,
					R.drawable.score_doll_1 + i);
		}
		for (int i = 0; i < 10; i++) {
			scoreNums[i] = BitmapFactory.decodeResource(resources,
					R.drawable.score_num_0 + i);
		}
		for (int i = 0; i < 3; i++) {
			rankSmallNums[i] = BitmapFactory.decodeResource(resources,
					R.drawable.score_rank_small1 + i);
			rankBigNums[i] = BitmapFactory.decodeResource(resources,
					R.drawable.score_rank_big1 + i);
		}
	}
	...
	public int breakRankNum() {
		...
		if (nowScore > nowUser.getFirstScore()) {
			which = 1;
			user.alterThird(userId, nowUser.getSecondName(),
					nowUser.getSecondScore());
			user.alterSecond(userId, nowUser.getFirstName(),
					nowUser.getFirstScore());
			user.alterFirst(userId, thirdEV.getText().toString(),
					nowScore);
			...
			user.alterThird(userId, nowUser.getSecondName(),
					nowUser.getSecondScore());
			user.alterSecond(userId, thirdEV.getText().toString(),
					nowScore);
			...
			which = 3;
			user.alterThird(userId, thirdEV.getText().toString(),
					nowScore);
	...
	class MyEditorActionListener implements TextView.OnEditorActionListener {
		...
		public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
			// TODO Auto-generated method stub
				...
				if(isfocus){
					if (breakNum == 1) {
						user.alterFirst(userId, firstEV.getText().toString(),
								nowScore);
						firstEV.clearFocus();
						firstEV.setFocusable(false);
					} else if (breakNum == 2) {
						user.alterSecond(userId, secEV.getText().toString(),
								nowScore);
						secEV.clearFocus();

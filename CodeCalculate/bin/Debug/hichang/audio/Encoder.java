...
public class Encoder implements Runnable {
	...
	private static final int MAXFALSECOUNT = 5;
		...
		while (this.isRecording() || bHangOn) {
			...
			synchronized (mutex) {
				...
				for (int i = 0; i < rawdata.length; i++) {
					final long v = rawdata[i];
		            sqsum += v * v;
				}
				
				// 超过185.0D即为有声音
				dVolume = (20.0D * Math.log10(Math.sqrt(sqsum / rawdata.length) / 2.0E-006D));
				...
				if (index < note.length) {
					...
					for (int k = previousRecordIndex + 1; k <= index; k++) {
						...
						//results[k] = (currentNote - (realNote - 12)) / 12;
						...
						if (realNote != 0) {
							...
							/*if (bCalculateMark) {
								mark[i] = 20*(mark[i] / (float) markPointCount[i]);
								currentMark = mark[i];
								markPointCount[i] = 0;
								bCalculateMark = false;
							}*/
